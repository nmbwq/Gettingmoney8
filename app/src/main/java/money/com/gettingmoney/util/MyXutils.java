...
//                params.addQueryStringParameter(entry.getKey(), entry.getValue());
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            @Override
            public void onError(Throwable ex, boolean isOnCallback) {
                Toast.makeText(con, "请求失败，请检查网络", Toast.LENGTH_SHORT).show();
                if(dialog!=null){
                    dialog.close();
                }
            }
    ...
    public void bindCommonImage(ImageView iv, String url, boolean option) {
        if (option) {
            options = new ImageOptions.Builder().setLoadingDrawableId(R.mipmap.icon_stub).setFailureDrawableId(R.mipmap.icon_erro).build();
    ...
    public void bindCircularImage(ImageView iv, String url, boolean option) {
        if (option) {
            options = new ImageOptions.Builder().setLoadingDrawableId(R.mipmap.icon_stub).setFailureDrawableId(R.mipmap.icon_erro).setCircular(true).build();
            x.image().bind(iv, url, options);
        } else {
            x.image().bind(iv, url);
        }
